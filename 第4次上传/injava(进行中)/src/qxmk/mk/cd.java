...
import static qxmk.mk.hide.yc;
import static qxmk.mk.md5.getMD5;
...
public interface cd {
    static void filecd(VALUE VALUE) throws IOException { //文件
        //保存上一次路径
        VALUE.qx = VALUE.lj;
        System.out.println("0.打开(调用vim)   1.复制   2.移动   3.删除   4.设置权限    5.重命名   6.隐藏(会影响路径)  06.取消选项6   7.格式化代码缩进(调用vim)   8.关于此文件  9.MD5");
        read(VALUE, "输入要选择的序号----");
        switch (VALUE.read) {
